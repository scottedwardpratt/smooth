...
\subsection{Overview}
The \sloppypar{\tt \$\{MY\_LOCAL\}} directory contains the source code for one program that performs an MCMC analysis. Once compiled, this program will invoke the emulator in its exploration of the model-parameter space. It will write the trace to file and perform and record some analysis of the trace the User might find useful. Even if the User chooses a different MCMC approach then the one provided with the {\it Smooth Emulator} software, the codes might prove useful in demonstrating how to invoke the emulator from within a C++ code. 

\subsection{Preparing for MCMC using {\it Smooth Emulator}}

The final step in Bayesian analyses is to generate MCMC traces through the parameter space. {\it Smooth Emulator} software is designed for the MCMC programs to be run from within the \sloppypar{\tt \$\{MY\_ANALYSIS\}} directory. Once the emulator is tuned and before it is applied to a Markov Chain investigation of the likelihood, the software needs know the experimental measurement and uncertainty. That information must be entered in the {\tt smooth\_data/Info/experimental\_info.txt} file. The file should have the format:
\vspace*{-12pt}{\tt
\begin{Verbatim}
 obsname1  value1    unc_exp1   unc_theory1
 obsname2  value2    unc_exp2   unc_theory2
 obsname3  value3    unc_exp3   unc_theory3
 obsname4  value4    unc_exp4   unc_theory4
  .
\end{Verbatim}
}\vspace*{-12pt}
...
\end{eqnarray*}
The likelihood behaves as $e^{LL}$. This form presumes that the uncertainties are Gaussian and that the uncertainties for the various observables are independent. The last term above derives from the normalization factor for the Gaussian.
...
\begin{Verbatim}[commandchars=\\\{\}]
 $\{MY_LOCAL\}/software% \textcolor{darkred}{make mcmc}
\end{Verbatim}
}\vspace*{-12pt}

One usually chooses to tune the emulator before running {\tt mcmc}. This would involve running the emulator and saving some of the emulator hyper-parameters. Alternatively, can insert the tuning into the main source code for {\tt mcmc} mentioned above.

\subsection{MCMC Options}

Next, one needs to edit the options file 
sloppypars{\tt\$\{MY\_ANALYSIS\}/smooth\_data/smooth\_parameters/mcmc\_parameters.txt}. An example file is:
\vspace*{-12pt}{\tt
\begin{Verbatim}
...
To run the provided MCMC program, to the analysis directory, and run the program {\tt mcmc}. Output should look something like this:
\vspace*{-12pt}{\tt
\begin{Verbatim}[commandchars=\\\{\}]
