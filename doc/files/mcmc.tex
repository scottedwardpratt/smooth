...
\subsection{Overview}
The {\tt \$\{SMOOTH\_LOCAL\}/software/MainPrograms/} directory contains the source code for the main program, {\tt smoothy\_mcmc\_main.cc}. When compiled it creates an executable, {\tt \$\{SMOOTH\_LOCAL\}/software/bin/smoothy\_mcmc}, which then performs an MCMC analysis. This program will invoke the emulator in its exploration of the model-parameter space. It will write the trace to file and perform and record some analysis of the trace the User might find useful. Even if the User chooses a different MCMC approach then the one provided with the {\it Smooth Emulator} software, viewing the source code might prove useful in demonstrating how to invoke the emulator from within a C++ code. 
...
As was the case with {\it Smooth Emulator}, each option has a default value. The {\tt CMCMC} class has a parameter map object. This map stores the options, and uses them to set the corresponding data in the {\tt CMCMC} object. The values of {\tt MCMC\_NBURN}, {\tt MCMC\_NTRACE} and {\tt MCMC\_NSKIP} are stored by the {\tt CMCMC} parameter map, but do not correspond to any members of the class. When the User wishes to set the number of trace points in the main program, they do so by changing the arguments to the {\tt PerformTrace(..)} function. The User can set these arguments through the parameter map functionality, which can provide a convenient way to set the arguments through the Options file. See Sec. \ref{sec:mcmcsource} below for an example of how this is used.The options are described below.
...
$\{MY_ANALYSIS\}% \textcolor{darkred}{$\{SMOOTH_LOCAL\}/bin/mcmc}
...
Finally, we review the source code in {\tt \$\{SMOOTH\_LOCAL\}/software/main\_programs/mcmc\_main.cc}:
{\tt
\begin{Verbatim}[commandchars=\\\{\}]
int main()\{
