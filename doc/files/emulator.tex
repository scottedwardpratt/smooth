...
\subsubsection{{\tt SmoothEmulator\_TrainingFormat (default SMOOTH)}}
...
\subsubsection{{\tt SmoothEmulator\_TestingFormat (default SMOOTH)}}
...
\subsubsection{{\tt Smooth\_Emulator\_FullModelTrainingParsFileName (default smooth\_data/FullModelRuns/modelpars\_surmise.txt}}
...
The executables should now appear in the {\tt \$\{MY\_LOCAL\}/bin/} directory. If one enters {\tt make} {\it program name} only that program  will be compiled. One can peruse the . Assuming the directory {\tt \$\{MY\_LOCAL\}/bin/} has been added to the User's path, the User may switch to the User's analysis directory, and enter the program name, e.g.
\vspace*{-12pt}{\tt \begin{Verbatim}[commandchars=\\\{\}]
...
As the title suggests, this program runs the MCMC exploration of model-parameter space using the emulator. The source code for the included main program, {\tt \$\{MY\_LOCAL\}/software/main\_programs/smoothy\_mcmc\_main.cc}, uses the functionality of the {\tt CSmoothMaster} class. The MCMC codes are discussed in detail in Sec. \ref{sec:mcmc}.
