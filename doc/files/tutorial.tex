...
\item Run a program {\tt smooth\_testvsfullmodel} which creates data files comparing emulator predictions to full-model runs at random points not used to train the emulator.
\item Run a Python plotting program that provides a visual comparison of the comparison in (6).
\item Run an MCMC program that produces a trace through model-parameter space that consistently represents the posterior probability by comparing emulator predictions to fake experimental data.
\item Run a Python plotting program that reads the MCMC trace and provides a visual representation of the model-parameter-space posterior.
\item Run a Python plotting program that uses covariances from the MCMC trace to display the resolving power matrix representing the sensitivity of any observable with respect to the constraint of any model parameter.
\end{enumerate}
...
\subsection{Selecting the Training Points with {\tt trainingpoint\_optimizer}}
...
Your results should look similar in principle, but should vary in detail because different random points were chosen.
