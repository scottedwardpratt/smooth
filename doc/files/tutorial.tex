...
{\tt \$\{SMOOTH\_LOCAL\}} & \parbox{5in}{Can be placed anywhere. Executables are store in \sloppypar{\tt \$\{SMOOTH\_LOCAL\}/bin} and main programs, and source codes for main\\ programs are found within {\tt \$\{SMOOTH\_LOCAL\}/software/main\_programs}\\}\\
...
$\{SMOOTH_HOME\}/software% cd $\{SMOOTH_LOCAL\}/software
...
\vspace*{-8pt}{\tt \begin{Verbatim}[commandchars=\\\{\}]
   ...
   $\{SMOOTH_LOCAL\}/software% \textcolor{darkred}{make}
\end{Verbatim}
}\vspace*{-8pt}

This will compile all the main source programs in {\tt \$\{SMOOTH\_LOCAL\}/software/MainPrograms/} and two programs used to generate files for the tutorial in {\tt \$\{SMOOTH\_LOCAL\}/software/TutorialPrograms/}. The repository was organized to encourage Users to edit any files in {\tt \$\{SMOOTH\_LOCAL\}/}. If the User wishes to restore any original files, a copy can be found at {\tt \$\{SMOOTH\_HOME\}/}.

Several executables should now appear in {\tt \$\{SMOOTH\_LOCAL\}/bin/}: \sloppypar{\tt trainingpoint\_optimizer, smoothy\_testattrainingpts, smoothy\_testvsfullmodel, smoothy\_calcobs, smoothy\_mcm}, which all involve the emulator. Two other executables, {\tt fakefullmodel} and {\tt fakeinfo} are only used for the tutorial. You might find it convenient to add \sloppypar{\tt \$\{SMOOTH\_LOCAL\}/bin} to their path. The reason these are compiled in a space separate from the main libraries, is that you may well wish to create your own main programs. This arrangement allows you to compile your own versions, while leaving the bulk of the source code unchanged. 
...
\vspace*{-8pt}{\tt \begin{Verbatim}[commandchars=\\\{\}]
   ...
   $\{MY_ANALYSIS\}% \textcolor{darkred}{$\{SMOOTH_LOCAL\}/bin/fakeinfo}
   \end{Verbatim}
}\vspace*{-8pt}
...
\vspace*{-8pt}{\tt \begin{Verbatim}[commandchars=\\\{\}]
${MY_ANALYSIS}% \textcolor{darkred}{${SMOOTH_LOCAL}/bin/trainingpoint_optimizer}
...
\vspace*{-8pt}{\tt \begin{Verbatim}[commandchars=\\\{\}]
   $\{MY_ANALYSIS\}% \textcolor{darkred}{$\{SMOOTH_LOCAL\}/bin/fakefullmodel}
   NPars=6, NObs=10
...
Here, you will run the program {\tt \$\{SMOOTH\_LOCAL\}/bin/smoothy\_testattrainingpts}. The corresponding source code for the first is {\tt \$\{SMOOTH\_LOCAL\}/software/MainPrograms/smoothy\_testattrainingpts\_main.cc},
\vspace*{-8pt}{\tt \begin{Verbatim}[commandchars=\\\{\}]
int main()\{
...
Running this code:
\vspace*{-8pt}{\tt \begin{Verbatim}[commandchars=\\\{\}]
$\{MY_ANALYSIS\}% \textcolor{darkred}{$\{SMOOTH_LOCAL\}/bin/smoothy_testattrainingpts}
----------- obs0 -------------
...
The source code, {\tt \{SMOOTH\_LOCAL\}/bin/smoothy\_mcmc\_main.cc}, is rather simple.
\vspace*{-8pt}{\tt \begin{Verbatim}[commandchars=\\\{\}]
int main()\{
   NBandSmooth::CSmoothMaster master;
   master.TuneAllY();
...
\vspace*{-8pt}{\tt \begin{Verbatim}[commandchars=\\\{\}] 
$\{MY_ANALYSIS\}/rhic% \textcolor{darkred}{\$\{SMOOTH_LOCAL\}/bin/smoothy_mcmc}
