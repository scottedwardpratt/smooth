...
At this point, the user should have established a personalized analysis directory by copying the {\tt \$\{GITHOME\_BAND\_SMOOTH\}/templates/} directory to a convenient location. For example,
{\tt
\begin{verbatim}
	% cp -r ${GITHOME_BAND_SMOTH}/templates /Users/CarlosSmith/mysmoothy
\end{verbatim}}
In this case {\tt \$\{MY\_LOCAL\}} will now refer to {\tt Users/CarlosSmith/mysmoothy/mylocal} and {\tt \$\{MY\_ANALYSIS\}} will now refer to a specific analysis directory, e.g.  {\tt Users/CarlosSmith/mysmoothy/myanalysis}.
...
Now the user can run {\tt Simplex Sampler}, which must be run from the analysis directory. The only output is the number of training points.
 {\tt
\begin{verbatim}
   ${MY_ANALYSIS}/rhic% ${MY_LOCAL}/bin/simplex
   NTrainingPts=28
\end{verbatim}
}
