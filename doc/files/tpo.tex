...
\subsubsection{{\tt TPO\_INCLUDE\_LAMBDA\_UNCERTAINTY} (default {\tt true})}
...
To run {\it Training Point Optimizer}, first make sure the program is compiled. To compile the programs, change into the {\tt \$\{SMOOTH\_LOCAL\}/main\_programs/} directory and enter the following command,

\vspace*{-8pt}{\tt \begin{Verbatim}[commandchars=\\\{\}]
   $\{SMOOTH_LOCAL\}/software% \textcolor{darkred}{cmake .}
   $\{SMOOTH_LOCAL\}/software% \textcolor{darkred}{make trainingpoint_optimizer}
\end{Verbatim}
}\vspace*{-8pt}
...
{\tt
\vspace*{-8pt}{\tt \begin{Verbatim}[commandchars=\\\{\}]
   $\{MY_ANALYIS\}% \textcolor{darkred}{$\{SMOOTH_LOCAL\}/bin/trainingpoint_optimizer}
\end{Verbatim}
}\vspace*{-8pt}
}
Here {\tt \$\{SMOOTH\_LOCAL\}/bin} is the path to where the User compiles the main programs into executables. 
...
Several additional methods can be seen by viewing the header file, {\tt \$\{SMOOTH\_HOME\}/software/include/msu\_smooth/trainingpoint\_optimizer.h}. However, the methods typically called are {\tt Optimize()}, {\tt WriteModelPars} with the correct options listed.
...
Further description of the {\tt parameterMap} class is provided in Sec. \ref{sec:parametermap}.
