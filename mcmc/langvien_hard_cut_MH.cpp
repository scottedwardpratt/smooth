...
vector<vector<double>> simulate_dynamics(const function<double(const vector<double>&)>& potential_func, double tau, double total_time, const vector<double>& initial_pos) {
    double dt = tau;
    long long int num_steps = static_cast<long long int>(total_time / dt);
    ...
    for (int i = 1; i < num_steps; ++i) {
        double current_x = traj[i - 1][0];
        vector<double> gradient = numerical_gradient(potential_func, traj[i - 1]);
        double random_force = dis(gen);


        double proposed_x = current_x - gradient[0] * dt + random_force;

        // Check for hard cutoffs and update position accordingly

        double accept_prob = min(1.0, exp(-potential_func({proposed_x}) + potential_func({current_x})));

        if (uniform_real_distribution<>(0, 1)(gen) < accept_prob && proposed_x > -1 && proposed_x < 1) {
            traj[i][0] = proposed_x;
        } else {
            traj[i][0] = current_x;  // Stay at the current position
        }
...
int main() {
    ...
    ofstream outFile("trajectory_hard_cut_MH.txt");
    if (outFile.is_open()) {
