...
void CMCMC::WriteTrace(){
   ...
   FILE *fptr;
   CLog::Info("writing trace, ntrace = "+to_string(ntrace)+"\n");
   fptr=fopen("smooth_data/MCMC/trace_theta.txt","w");
   ...
   fclose(fptr);
   fptr=fopen("smooth_data/MCMC/trace_X.txt","w");
   for(itrace=0;itrace<ntrace;itrace++){
      for(ipar=0;ipar<NPars;ipar++){
...
void CMCMC::ReadTrace(){
   ...
   FILE *fptr;
   fptr=fopen("smooth_data/MCMC/trace.txt","r");
   while(!feof(fptr)){
      for(ipar=0;ipar<NPars;ipar++){
